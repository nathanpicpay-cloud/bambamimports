...
export const ProductDetails: React.FC = () => {
  ...
  return (
    <div className="min-h-screen py-24 md:py-10 relative overflow-hidden">
      {/* Background Decor */}
      ...
      <div className="max-w-7xl mx-auto px-4 md:px-6 lg:px-8">
        <Link to="/loja" className="inline-flex items-center text-slate-500 hover:text-white transition-colors mb-6 md:mb-8 font-medium">
        ...
        <div className="grid lg:grid-cols-2 gap-8 lg:gap-24 items-start">
          ...
          <div className="space-y-6 animate-fade-in-up">
            <div className="aspect-square bg-white/5 backdrop-blur-xl rounded-[2rem] md:rounded-[3rem] border border-white/5 shadow-2xl shadow-black/20 p-6 md:p-10 relative overflow-hidden group">
               ...
               {product.originalPrice && (
                 <div className="absolute top-4 left-4 md:top-6 md:left-6 bg-brand-500 text-slate-950 text-xs font-bold px-3 py-1.5 rounded-full z-20 shadow-lg shadow-brand-500/20">
                   {Math.round(((product.originalPrice - product.price) / product.originalPrice) * 100)}% OFF
                 </div>
               )}
            </div>
            
            <div className="flex justify-start md:justify-center gap-4 overflow-x-auto pb-2 md:pb-0 scrollbar-hide">
              {product.images.map((img, i) => (
                <button 
                  key={i} 
                  onClick={() => setActiveImage(img)}
                  className={`flex-shrink-0 w-16 h-16 md:w-20 md:h-20 rounded-2xl bg-white/5 border-2 overflow-hidden p-2 transition-all duration-300 ${activeImage === img ? 'border-brand-500 scale-110 shadow-lg shadow-brand-500/10' : 'border-transparent opacity-50 hover:opacity-100'}`}
                >
          ...
          {/* Info Section */}
          <div className="animate-fade-in-up animate-delay-200 pt-0 md:pt-4">
            ...
            <h1 className="text-3xl md:text-5xl font-black text-white mb-2 md:mb-4 tracking-tight leading-tight">{product.name}</h1>
            ...
            <p className="text-slate-300 text-base md:text-lg leading-relaxed mb-8 md:mb-10">{product.description}</p>

            {/* Price & Action Card */}
            <div className="bg-slate-900/60 backdrop-blur-md rounded-[2rem] p-6 md:p-8 border border-white/5 shadow-xl shadow-black/30 mb-8 md:mb-10 relative overflow-hidden">
              ...
              <div className="relative z-10">
                ...
                <div className="flex flex-col sm:flex-row sm:items-baseline gap-2 sm:gap-4 mb-2">
                  <span className="text-4xl md:text-5xl font-black text-brand-400 tracking-tight">
                ...
                <div className="space-y-3">
                  <Button onClick={() => addToCart(product)} size="lg" fullWidth className="shadow-brand-500/25 h-12 md:h-14 text-base md:text-lg">
                  ...
                  <a href={`https://wa.me/5575999999999?text=Tenho interesse no ${product.name}`} target="_blank" rel="noreferrer" className="block w-full">
                    <Button variant="outline" fullWidth className="h-12 md:h-14 bg-transparent border-slate-700 text-slate-300 hover:bg-white/5 hover:border-brand-500 hover:text-brand-400">Negociar no WhatsApp</Button>
                  </a>
                </div>
              </div>
            </div>
